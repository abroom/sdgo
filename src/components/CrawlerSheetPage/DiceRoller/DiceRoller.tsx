...
export const DiceRoller = ({ close }: { readonly close: () => void }) => {
	const [customDice, setCustomDice] = useState<number>(6);
	...
	return (
		<div className="fixed inset-0 flex flex-col gap-4 sm:gap-8 p-4 sm:p-8 bg-(--color-primary)">
			<div className="flex-grow grid grid-cols-1 sm:grid-cols-2 gap-2 overflow-hidden">
				<div className="order-last sm:order-first border border-(--color-primary-3) rounded-lg flex-grow max-h-50% p-2 grid grid-cols-2 sm:gris-cols-1 gap-2 overflow-auto overscroll-contain">
					...
					<div className="col-span-full flex items-center gap-2">
						<label className="text-lg" htmlFor="custom-dice">
							Custom
						</label>
						<input
							id="custom-dice"
